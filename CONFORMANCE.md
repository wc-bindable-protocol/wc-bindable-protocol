...
async function testInvalidAckOptions() {
  ...
  return err.code; // expected: "WC_BINDABLE_INVALID_ACK_OPTIONS"
}
```
...
- `testInvalidAckOptions()` resolves to `"WC_BINDABLE_INVALID_ACK_OPTIONS"` (NOT `WC_BINDABLE_PROTOCOL_ERROR` — nothing on the wire is malformed; this is a call-site argument validation failure, symmetric with `WC_BINDABLE_INVALID_JSON_VALUE` for the `value` / `args` validation gate). The returned `Promise` MUST be synchronously already-rejected (not a deferred microtask rejection), and the underlying `Error` MUST be `RangeError`-shaped per [SPEC-extensions.md § AckOptions](SPEC-extensions.md#ackoptions).
