...
**Independence sub-case.** Two `bind(..., { syncOn: "define" })` calls on the same un-upgraded element, one of which is cancelled before the definition arrives: the surviving bind MUST still register and deliver on definition. Each `bind()` owns its own wait and its own disposal state.
