...
- `testInvalidAckOptions()` resolves to `{ code: "WC_BINDABLE_INVALID_ACK_OPTIONS", name: "RangeError" }`. The `code` value MUST NOT be `WC_BINDABLE_PROTOCOL_ERROR` — nothing on the wire is malformed; this is a call-site argument validation failure, symmetric with `WC_BINDABLE_INVALID_JSON_VALUE` for the `value` / `args` validation gate. The `name` value MUST be exactly `"RangeError"` per the RangeError-shaped definition in [SPEC-extensions.md § AckOptions](SPEC-extensions.md#ackoptions) — the spec's minimum requirement is `error.name === "RangeError"` regardless of whether the implementation actually constructs the rejection with `new RangeError(...)` (cross-realm / non-JS implementations are not required to preserve `instanceof RangeError`, so the test asserts on `name` rather than prototype identity). The returned `Promise` MUST be synchronously already-rejected (not a deferred microtask rejection) per [SPEC-extensions.md § AckOptions](SPEC-extensions.md#ackoptions); vector 30's `try / await / catch` shape does not distinguish synchronous from microtask-deferred rejection — see the "Synchrony of `setWithAckOptions` / `invokeWithOptions` invalid-`timeoutMs` rejection" entry in [§ What this list does NOT cover](#what-this-list-does-not-cover) for the future-work vector that would tighten that check.
