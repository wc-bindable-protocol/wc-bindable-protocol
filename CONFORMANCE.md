...
>
> **Harness note.** The reference test environment for this repository (`happy-dom` 20.x) does **not** implement custom element upgrade at all: after `customElements.define()`, an instance created beforehand keeps `constructor === HTMLElement`, and neither insertion nor `customElements.upgrade()` swaps its prototype. It additionally *replaces the node* for an element already in the document rather than upgrading it in place. This vector is written against real browser semantics. A harness on such an environment MUST either run it in a real browser or simulate the upgrade explicitly (swap the instance's prototype at `define()` time) — the deferred path depends only on the observable result of the upgrade, namely `target.constructor.wcBindable` becoming readable.
>
> The reference implementation does both: `packages/core/tests/index.test.ts` simulates the swap under vitest, and `packages/core/tests/integration/syncOnDefine.spec.ts` runs the same cases against a real browser under Playwright (`npm run test:integration --workspace @wc-bindable/core`). Simulation alone pins the consumer half of the contract; the platform half — in-place upgrade, node identity, and the `customElements.upgrade()` MUST for detached targets — needs the browser.
...
## Extension 4 — Composition vectors

These vectors test the **Composition profile** ([SPEC-extensions.md § Extension 4 — Composition](SPEC-extensions.md#extension-4--composition)). They map 1:1 to the draft vectors in [COMPOSITE.md § Draft conformance vectors](COMPOSITE.md) and are now normative against Extension 4 (which is authoritative; where a vector below disagrees with Extension 4, the spec section wins).

**Composite-tier scoping.** The "Applies to" column uses the **composite profile tiers** (distinct from the SPEC.md `{1O/1P/2/3}` facets above), as read from the tier-claim object `Symbol.for("wc-bindable.composite.tiers")`:

| Composite tier | What it means |
|---|---|
| **`[T1]`** | Every composed shell (base observation surface). Run by all shells. |
| **`[T2]`** | Only when the shell claims the local-facade tier (`localFacade: true`). |
| **`[T3]`** | Only when the shell claims the Extension-1-capable tier (`extension1: true`). |

A shell claiming "T1 + T3 only" runs the `[T1]` and `[T3]` vectors and MAY skip `[T2]`-only ones. Some vectors carry an additional conditional gate (remote compatibility, cascading ownership, listener detach-on-reconnect) noted inline. Vectors are at starter-set (table-row) granularity, matching how they are defined in COMPOSITE.md; the test-case text plus the Extension 4 reference is the runnable target.

| # | Applies to | Test case | Extension 4 § |
|---|---|---|---|
| C1 | `[T1]` | Synthesized declaration is accepted by `getWcBindableDeclaration()` | [§ 2](SPEC-extensions.md#2-declaration-shape) |
| C2 | `[T1]` | `bind(shell, onUpdate)` delivers initial values for exposed properties | [§ 5](SPEC-extensions.md#5-initial-synchronization) |
| C3 | `[T1]` | A source event re-emits exactly one shell event for the mapped composed property | [§ 6](SPEC-extensions.md#6-event-rewriting) |
| C4 | `[T1]` | A source custom getter is applied once, not copied and re-applied by the shell declaration | [§ 7](SPEC-extensions.md#7-getter-semantics) |
| C5 | `[T1]` | Two source properties sharing one source event become distinct shell property events, **both** dispatched; three-phase (extract→commit→dispatch): (a) a getter reading `shell["A"]` sees pre-event `A`; (b) a listener on `A` reading `shell["B"]` sees `B`'s committed value; (c) a property whose getter threw is neither committed nor dispatched while siblings are | [§ 6](SPEC-extensions.md#6-event-rewriting) |
| C6 | `[T2]` | Input assignment on the shell delegates to the correct source property | [§ 8](SPEC-extensions.md#8-inputs) |
| C7 | `[T2]` | Command invocation on the shell delegates to the correct source method and preserves async return behavior | [§ 9](SPEC-extensions.md#9-commands) |
| C8 | `[T1]` | Duplicate composed property names are rejected at construction | [§ 4](SPEC-extensions.md#4-collision-policy) |
| C9 | `[T1]` *(conditional — remote compatibility enabled)* | The full remote-reserved-name set is rejected (`@wc-bindable/` case-insensitive prefix **and** `__proto__` / `constructor` / `prototype`), referenced from the wire profile, not re-listed; a non-remote T1 shell skips | [§ 4](SPEC-extensions.md#4-collision-policy) + [§ Reserved names](SPEC-extensions.md#reserved-names) |
| C10 | `[T1]` | Final teardown removes every installed source listener; a shell that detaches on non-terminal disconnect does so only per its declared lifecycle, and a shell that keeps listeners across disconnect does not leak at end of life | [§ 10](SPEC-extensions.md#10-lifecycle-and-teardown) |
| C11 | `[T1]` | A remote-proxy source reports `N in shell === false` before first sync, `true` after; `bind()` delivers no premature initial value | [§ 5](SPEC-extensions.md#5-initial-synchronization) |
| C12 | `[T1]` | A source getter yielding top-level `undefined` (or `null`) delivers that exact value — not replaced by `sourceEvent.detail`, not coerced to `null` by a `CustomEvent.detail` boundary | [§ 7](SPEC-extensions.md#7-getter-semantics) |
| C13 | `[T2]` | A name colliding across `properties` / `inputs` / `commands` is rejected at construction when the shell claims T2 | [§ 4](SPEC-extensions.md#4-collision-policy) |
| C14 | `[T1]` | The synthesized declaration is immutable for the target's lifetime; a source / expose-map change produces a new shell target rather than mutating the existing one | [§ 11](SPEC-extensions.md#11-dynamic-reconfiguration) |
| C15 | `[T1]` | The declarative API does not expose a shell instance until every source custom element is defined (undefined source tag is a construction error or awaited via `customElements.whenDefined()`) | [§ 11](SPEC-extensions.md#11-dynamic-reconfiguration) |
| C16 | `[T2]` / `[T3]` | Source failures propagate per surface: `[T2]` a throwing source **input setter** propagates synchronously; `[T2]` a throwing source **command** preserves its completion shape (sync throw stays sync, promise rejection stays rejection); `[T3]` the Extension-1 surface surfaces both as promise rejections carrying the error envelope | [§ 12](SPEC-extensions.md#12-delegation-and-error-propagation) |
| C17 | `[T3]` | A T3-only shell routes `set` / `invoke` to the correct source member **without** materializing `shell[name]` as an assignable/callable member, and a name shared between `inputs` and `commands` is accepted (the § 4 cross-surface rule does not apply to T3) | [§ 4](SPEC-extensions.md#4-collision-policy) + [§ 8](SPEC-extensions.md#8-inputs) |
| C18 | `[T2]` | A composed shell exposed through `@wc-bindable/remote` for writable inputs / invokable commands requires the local facade (T2): `RemoteShellProxy`-driven `set` / `cmd` reach the mapped source; a T1-metadata-only shell does not wire them | [§ Profile tiers](SPEC-extensions.md#profile-tiers) + [§ Remote interop](SPEC-extensions.md#remote-interop) |
| C19 | `[T1]` | Reconnect re-sync per the declared policy — **Unconditional** (every known/present property dispatches), **Changed-only** (only `Object.is`-different values dispatch; `NaN` unchanged, `+0`/`-0` changed), or **Documented silence** (no re-sync events) | [§ 10](SPEC-extensions.md#10-lifecycle-and-teardown) |
| C20 | `[T1]` | When two properties share one source event and the **first**'s getter throws, the second is still dispatched, and the failure is reported through host error reporting — not swallowed, not suppressing the sibling | [§ 6](SPEC-extensions.md#6-event-rewriting) + [§ 12](SPEC-extensions.md#12-delegation-and-error-propagation) |
| C21 | `[T3]` | A T3 shell whose source is itself Extension-1-capable delegates `setWithAck` / `invoke` end-to-end (source ack/rejection propagates); over a plain local source it wraps assignment / method call in Extension-1 semantics | [§ 8](SPEC-extensions.md#8-inputs) + [§ 9](SPEC-extensions.md#9-commands) |
| C22 | `[T3]` | A T3 shell exposes the **full** Extension 1 surface incl. `setWithAckOptions` / `invokeWithOptions` and `dispose`; after `dispose()`, `set` throws synchronously while the acknowledged calls each reject | [§ Profile tiers](SPEC-extensions.md#profile-tiers) + [§ 10](SPEC-extensions.md#10-lifecycle-and-teardown) |
| C23 | `[T3]` | Source capability is classified per delegation kind in **auto** using a **positive marker**, not method names: full subset **and** supported marker → capable; none → plain-local; some-but-not-all → rejected; full method set but **no** marker → **ambiguous** and rejected (incl. the full-collision case where a local target has methods named `set`/`setWithAck`/`setWithAckOptions` or `invoke`/`invokeWithOptions`); input- and command-kinds classified independently | [§ 8](SPEC-extensions.md#8-inputs) |
| C24 | `[T3]` | The explicit override changes classification per kind: a local source whose method is literally named `invoke` / `setWithAck` is not rejected when its kind is pinned **local**, while another kind stays **extension1**; **extension1** opts in a genuine-but-unmarked source and rejects one missing the required subset; whole-source mode is a shorthand, per-kind value takes precedence | [§ 8](SPEC-extensions.md#8-inputs) |
| C25 | `[T3]` | `setWithAck` / `setWithAckOptions` against a plain local (synchronous) source reject on a pre-aborted signal and on invalid options before assigning, but a pending `timeoutMs` does NOT produce `WC_BINDABLE_TIMEOUT` for the synchronous assignment; `invoke()` awaiting a local thenable DOES honor `timeoutMs` | [§ 8](SPEC-extensions.md#8-inputs) + [§ 9](SPEC-extensions.md#9-commands) |
| C26 | `[T1]` | Source ownership — a source shared between two shells (or referenced after one shell's `dispose()`) is NOT torn down by that shell: it removes only its own listeners; the source remains usable | [§ 10](SPEC-extensions.md#10-lifecycle-and-teardown) |
| C27 | `[T3]` | A late source settlement after `dispose()` is dropped (optionally logged); it does not re-settle or otherwise surface the shell-side promise | [§ 10](SPEC-extensions.md#10-lifecycle-and-teardown) |
| C28 | `[T1]` *(conditional — only if cascading ownership is offered)* | Cascading dispose is resilient: when `ownsSources` is enabled, a source whose `dispose()` throws does not prevent disposing the rest; an implementation that does not offer the opt-in skips this vector | [§ 10](SPEC-extensions.md#10-lifecycle-and-teardown) |
| C29 | `[T1]` | The tier claim is readable from `shell[Symbol.for("wc-bindable.composite.tiers")]` returning `{ protocol: "wc-bindable.composite", version: 1, localFacade, extension1 }` for the instance; reading does not throw; a tool MUST NOT render a declared input/command as assignable/invokable until the surface confirms the tier; consumer reads `version` first, ignores unknown fields; a validator does not reject for extra fields; a private field uses a namespaced/vendor key | [§ Tier-claim discovery surface](SPEC-extensions.md#tier-claim-discovery-surface) |
| C30 | `[T3]` | A T3 acknowledged call is backed by a shell-owned pending entry; `abort`/`timeout` ownership follows source kind (Extension-1-capable: forward `AckOptions` and **mirror** — no double handling; plain local: shell owns locally); `dispose()` is always the shell's terminal — rejects shell-owned pending in caller order with `WC_BINDABLE_DISPOSED`, drops later source settlement, never disposes/cancels the source, wins a same-turn race; each terminal single-settlement, first-wins | [§ 12](SPEC-extensions.md#12-delegation-and-error-propagation) |
| C31 | `[T3]` | A plain-local-source T3 call normalizes failures to the Extension 1 contract: a non-`Error` throw → `name === "NonErrorThrow"` with safely-stringified `message`; an application throw carries no invented protocol code (`WC_BINDABLE_REMOTE_THROW` or unset); shell-synthesized protocol errors carry their registered `WC_BINDABLE_*` code — whereas `[T2]` re-raises the raw synchronous throw unaltered | [§ 12](SPEC-extensions.md#12-delegation-and-error-propagation) |
| C32 | `[T1]` | A source event re-dispatches synchronously within the same call stack — not deferred to a microtask/task, not batched or de-duplicated; a `bind()` consumer sees the update on the same turn it would binding the source directly | [§ 6](SPEC-extensions.md#6-event-rewriting) |
| C33 | `[T1]` | For each dispatched composed property, `shell[N]` and `N in shell` reflect the **new** value before the shell event fires (a listener reading `shell[N]` synchronously observes the new value) | [§ 6](SPEC-extensions.md#6-event-rewriting) |
| C34 | `[T1]` | A non-T2 shell exposed through `@wc-bindable/remote` is given a **remote-facing target whose `constructor.wcBindable` omits `inputs` / `commands`** (generated isolated constructor / wrapper), so the consumer renders no set/invoke affordance while properties still sync; the original shell keeps its inputs/commands for local tooling; passing the original T1 shell directly to `RemoteShellProxy` is non-conformant | [§ Remote interop](SPEC-extensions.md#remote-interop) |
| C35 | `[T1]` | `remoteCompatible: true` means "this object's current declaration can be handed directly to `RemoteShellProxy`" and sits only on a directly-exposable object (a T2 shell or a remote-facing projection), never a non-T2 original; such an object satisfies the reserved-name set and `JsonValue` serializability; absent/`false` means "do not pass me directly" and a runner skips the remote-only vectors; "the implementation can produce a projection" is a separate out-of-band claim | [§ Discovering remote compatibility](SPEC-extensions.md#discovering-remote-compatibility) |
| C36 | `[T3]` | The Extension-1 marker is a versioned object, not a bare flag: `{ protocol: "wc-bindable.extension1", version: 1, inputs, commands }` honored per kinds; an unsupported `version` or unrecognized `protocol` is **failed closed** (not marker-capable → ambiguous under `auto`); the bare-boolean shorthand is optional; the marker follows the tier-claim access rules incl. **throwing read ⇒ unavailable** | [§ 8](SPEC-extensions.md#8-inputs) |
| C37 | `[T1]` *(conditional — only if the shell detaches/reinstalls listeners across reconnect)* | Such a shell exposes its `reconnectResync` policy (`"unconditional" \| "changed-only" \| "silent"`) on the tier-claim object; a shell that never detaches MAY omit it | [§ 10](SPEC-extensions.md#10-lifecycle-and-teardown) |
| C38 | `[T1]` | Fail-closed on an unsupported claim: a consumer facing an unrecognized `protocol` or unsupported `version` on the tier-claim object treats the claim as **unavailable** — renders no execution affordance from `localFacade`/`extension1` and ignores optional fields except for diagnostics | [§ Tier-claim discovery surface](SPEC-extensions.md#tier-claim-discovery-surface) |
| C39 | `[T1]` / `[T3]` | A markerless current/legacy `RemoteCoreProxy` source: `[T1]` can still be **observed** without any override; `[T3]` delegating **inputs/commands** to it is ambiguous under `auto` and MUST require an explicit `extension1` override (never auto-delegated) | [§ 8](SPEC-extensions.md#8-inputs) + [§ Remote interop](SPEC-extensions.md#remote-interop) |
| C40 | `[T1]` *(conditional — remote compatibility enabled)* | A property whose value becomes non-`JsonValue` is **dropped at the wire boundary** (never forwarded as a `null`/`{}`-corrupted value) and the drop is **reported, not silently swallowed** by whichever layer performs it (the remote layer's existing producer-side warning/drop satisfies this; the composite reports only when it drops the value itself; not double-reported); local `bind()` consumers still observe the real JS value; inbound `set.value` / `cmd.args` and command return values surface `WC_BINDABLE_INVALID_JSON_VALUE` / `WC_BINDABLE_INVALID_RETURN_VALUE` from the producer gate | [§ Discovering remote compatibility](SPEC-extensions.md#discovering-remote-compatibility) |
| C41 | `[T3]` | Same-turn dispose race: `dispose()` synchronously marks every not-yet-settled shell-owned pending entry `Disposed`/`WC_BINDABLE_DISPOSED` at the call site, so a source promise that resolved but whose mirroring continuation was still queued is dropped (`dispose()` wins); an entry whose settlement callback already ran before `dispose()` stays terminal | [§ 12](SPEC-extensions.md#12-delegation-and-error-propagation) |
| C42 | `[T3]` | Default-timeout policy is uniform and documented, and **never drops the caller's `signal`**: a no-default-timeout call under **Normalize** is delegated as `source.setWithAckOptions(M, v, { ...opts, timeoutMs: shellDefault })` / `source.invokeWithOptions(...)` (merging, preserving `signal`); a `signal`-but-no-`timeoutMs` call stays abortable through the composite; under **Inherit-and-document** the timeout inherits the source default but `signal` is still forwarded; an explicit caller `timeoutMs` (incl. `0`) is forwarded unchanged | [§ 8](SPEC-extensions.md#8-inputs) + [§ 9](SPEC-extensions.md#9-commands) |
| C43 | `[T1]` | The tier-claim symbol resolves as an **own** property/getter (not prototype-chain only), every read yields the same logical claim, and the returned object is read-only (SHOULD be frozen) so two inspections never disagree | [§ Tier-claim discovery surface](SPEC-extensions.md#tier-claim-discovery-surface) |
| C44 | `[T3]` | Caller-order is scoped to a single source / logical channel: the shell begins delegation in call order; calls to the **same** source inherit that source's ordering; calls to **different** sources get **no** guaranteed producer-side relative order (cross-source sequencing is the caller's responsibility) | [§ 8](SPEC-extensions.md#8-inputs) |
| C45 | `[T1]` | A source event firing **reentrantly** from inside a shell-event listener is processed immediately and nested (its full extract→commit→dispatch cycle completes before the outer cycle's remaining listeners resume), not queued; the nested cycle sees the outer cycle's already-committed values; the cache is never rolled back | [§ 6](SPEC-extensions.md#6-event-rewriting) |
| C46 | `[T1]` / `[T2]` / `[T3]` | Fixed-API collision rejected at construction: `[T1, all tiers]` `constructor`, `addEventListener` / `removeEventListener` (and `dispatchEvent` when exposed as a dispatchable source), `__proto__` / `prototype` (MUST when remote-compatible, SHOULD otherwise); `[T3]` `set` / `setWithAck` / `setWithAckOptions` / `invoke` / `invokeWithOptions` / `dispose`; `[T2]` an own facade member it would overwrite; a T2+T3 shell applies the base set plus both per-tier sets | [§ 4](SPEC-extensions.md#4-collision-policy) |
| C47 | `[T1]` | `remoteCompatible: true` is a responsibility claim, not a static payload guarantee: a tool MUST NOT read it as "all emitted values are pre-guaranteed `JsonValue`", only as "directly `RemoteShellProxy`-able and bound by the wire profile's reserved-name + `JsonValue` failure semantics" | [§ Discovering remote compatibility](SPEC-extensions.md#discovering-remote-compatibility) |
| C48 | `[T1]` | Alias-transparent fan-out: if the same underlying source-event **occurrence** is exposed through multiple aliases / source ids, the affected composed properties still behave as one § 6 three-phase cycle — a listener on `a.x` reading `shell["b.y"]` after the shared commit point observes `b.y`'s committed value, not an implementation-defined interleaving | [§ 3](SPEC-extensions.md#3-name-mapping) + [§ 6](SPEC-extensions.md#6-event-rewriting) |
| C49 | `[T1]` | Source subscription ownership belongs to the shell, not downstream `bind(shell, …)` consumers: a shell with no current consumers continues its § 10 source-listener lifecycle, and a later consumer observes state derived from that shell-owned tracking rather than a fresh consumer-triggered subscription | [§ 10](SPEC-extensions.md#10-lifecycle-and-teardown) |
| C50 | `[T3]` | A composed shell that claims T3 and is used as a source for a parent composite SHOULD advertise a matching `Symbol.for("wc-bindable.extension1")` marker so the parent's `auto` classification accepts it without an explicit override; if both the marker and the tier claim are present, they agree on whether the child is Extension-1-capable | [§ 8](SPEC-extensions.md#8-inputs) |
