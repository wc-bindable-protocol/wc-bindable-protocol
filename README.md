...
>
> **Note for third-party implementers.** The reference implementation `@wc-bindable/remote` 0.7.x has a documented divergence from one of the current conformance vectors — remote `undefined` update delivery is observed as `null` by `bind()` callbacks because `CustomEvent.detail` is WebIDL-coerced. The spec is authoritative; the implementation is expected to close this gap in a future release. See [CONFORMANCE.md vector 6](CONFORMANCE.md#6-remote-undefined--absent-updatevalue-delivers-undefined-not-null) for the full callout. CONFORMANCE.md is the single source of truth for the current divergence list (each affected vector carries a "Known divergence in the reference implementation" callout); README does not enumerate the rest to avoid drift as vectors are added or fixed.
...
> **Layer 3 narrows the value model.** Layer 1 (local observation) carries whatever JavaScript value the producer exposes — `Date`, `Map`, `Set`, `BigInt`, typed arrays, class instances, functions, cyclic objects, anything. Layer 2 only declares interface metadata (`inputs` / `commands`) and does not add a value transport model of its own. The remote transport profile (Layer 3) narrows **ordinary wire payloads** to **`JsonValue`** (a recursive JSON-shape type) at every wire crossing — where "ordinary wire payloads" means every value-carrying field on the wire **except** the out-of-band top-level observable-property `undefined` markers covered in the carve-out below: `Date`, `Map`, `Set`, `BigInt`, typed arrays, class instances, functions, symbols, non-finite numbers, and cyclic references are all rejected by Layer 3 validation, not silently coerced. (This exclusion set is kept in sync with [SPEC.md § Protocol Model and Assumptions → Value model](SPEC.md#protocol-model-and-assumptions); the README and SPEC enumerations cover the same set of types so the README never reads as a strictly shorter authoritative list, though the ordering and wording around them are not promised to match character-for-character.) Components that expose those shapes locally need an explicit serialization boundary (encode to plain JSON objects at the producer, decode at the consumer) before they can be used through `@wc-bindable/remote`.
>
> **`undefined` is a special case, not simply "rejected".** `JsonValue` itself cannot contain `undefined` — so `undefined` nested inside an object property, array element, command argument, or return value remains non-`JsonValue` and IS rejected by Layer 3 validation, same as `Date` / `Map` / etc. above. But a *top-level observable property* whose current value is `undefined` is a first-class wire concept and is represented out-of-band: in the initial `sync` envelope via the `undefinedProperties` list, and in subsequent `update` envelopes via an absent `value` field. The protocol deliberately preserves "the producer's property is currently `undefined`" across the wire — what it does not allow is `undefined` as a value buried inside a larger JSON shape.
>
> The full algorithm and the `JsonValue` definition live in [SPEC-extensions.md § Extension 2 → Design invariants invariant 3](SPEC-extensions.md#extension-2--wire-format-remote-proxying); the out-of-band undefined-preservation rule lives in [SPEC-extensions.md § Undefined enumeration](SPEC-extensions.md#undefined-enumeration); the protocol-level model split is summarized in [SPEC.md § Protocol Model and Assumptions → Value model](SPEC.md#protocol-model-and-assumptions).
...
For DOM elements that have not yet been connected when `bind()` is called, pass `{ syncOn: "connect" }` to defer the initial read until `connectedCallback` has run. Framework adapters that bind from a mounted-element lifecycle hook (React `useEffect`, Vue `onMounted`, Angular `AfterViewInit`, etc.) do not need it, because the host already guarantees the element is attached. The imperative binders this repository ships for VanJS / MobX / RxJS / Signals do pass it internally, so callers do not have to sequence `appendChild()` and `binder.bind(el)` manually; this is a guideline rather than a spec MUST.

> **What the adapters in this repository pass.** Every adapter defers **discovery** by default — `syncOn: "define"` for the lifecycle-hook adapters, and `["define", "connect"]` for the four imperative binders, which are handed a detached element and so need both deferrals. Each adapter accepts a `syncOn` of your own if you want the historical `"call"` behavior back; see the *Late-defined elements* section in the adapter's README.
...
For remote targets (`@wc-bindable/remote`), the proxy layer is a **protocol layer, not a security boundary**: authentication, authorization, rate limiting, and **application-level** payload validation beyond the wire-shape `JsonValue` check are the responsibility of the layer that owns the transport. (Wire-shape `JsonValue` validation is the protocol layer's MUST on both sides — see [SPEC-extensions.md § Design invariants](SPEC-extensions.md#design-invariants); domain validation of what each `input` / `command` accepts is what lives above that.) Do not expose a Core directly to an untrusted peer without those guardrails. `getter` functions are NEVER transported as code — they run on the trusted side and only extracted values cross the wire.
...
- **Prefer `setWithAck` over `set` when an `invoke` depends on the prior write.** `set` is fire-and-forget (at-most-once, can be silently dropped on a transient outage); `setWithAck` waits for the JS-level assignment on the trusted side. **Security-relevant inputs (URL, HTTP method, auth state, feature flag, anything token-like) MUST use `await setWithAck()` before a dependent `invoke()`** — a silently-dropped `set("authToken", …)` followed by `invoke("savePayment")` runs the command against whatever the producer last saw, with no error on either side. The proxy's queue is FIFO but not transactional: it does NOT cancel the `invoke` when the prior `set` is lost. Full caveat: [SPEC-extensions.md § Methods](SPEC-extensions.md#methods), the `set` row.
