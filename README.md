...
> **The "one snippet" framing is a Layer-1 conceptual demo, not a Level 2 conformance bar.** The snippet above shows what `bind()` *does* in the happy path. A drop-in replacement for `@wc-bindable/core` (i.e. a third-party implementation claiming Level 2 conformance per [SPEC.md § Conformance Levels](SPEC.md#conformance-levels)) MUST perform the full schema validation that `getWcBindableDeclaration()` is specified to perform — descriptor-shape checks, name-uniqueness within `properties` / `inputs` / `commands`, the MUST-NOT-throw guard against hostile constructors / Proxy traps, and the exception-safe teardown rules in [SPEC.md § Teardown Contract](SPEC.md#teardown-contract). The reference implementation is intentionally a few hundred lines, not "tens", because Discovery == bindability is normative — `isWcBindable(t) === true` MUST imply `bind(t, ...)` will install listeners. A literal tens-of-lines impl that skips the validation will silently no-op against malformed declarations and break the consumer's `isWcBindable`-gated logic. See [SPEC.md § Discovery API](SPEC.md#discovery-api) and the [§ Appendix: Design rationale notes](SPEC.md#appendix-design-rationale-notes) for the history of why this rule is normative.
...
> **Behavior vs. schema validation.** Core does not execute or interpret `inputs` / `commands` at all — only `properties` drive `bind()`. Discovery still validates the schema of `inputs` / `commands` when present, however, so a malformed `attribute` or `async` field invalidates the declaration as a whole and — assuming `onUpdate` itself is valid (the reference implementation throws a `TypeError` synchronously before discovery when `onUpdate` is not a function; the spec rule is SHOULD, see [SPEC.md § onUpdate validity](SPEC.md#onupdate-validity) for why the deferred-detection alternative is discouraged) — `bind()` returns its no-op cleanup. The split is intentional: core stays narrow on *runtime behavior*, but the discovery contract is *well-formedness* — which has to be complete for downstream tools (codegen, remote proxy, devtools) to trust what they see.
...
**Discovery itself touches the target.** Even reading `target.constructor.wcBindable` to discover a declaration goes through JavaScript property access, which means a hostile target may expose `constructor`, `wcBindable`, or any descriptor field through `Proxy` traps or accessor properties that have side effects on read. The core's `getWcBindableDeclaration()` helper wraps its **entire validation body** in a single `try / catch` so a throw from any access during validation funnels to a clean `return undefined` — but each Schema-typed field is snapshotted into a local *once* before being validated (so a hostile accessor that returns different values on successive reads cannot present one shape to the validator and another to downstream consumers), and the helper still cannot prevent side effects from running. Consumers should bind only to targets they intentionally loaded; the helper's no-throw posture is a safety net for already-trusted code, not a sandbox.
