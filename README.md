...
> **Behavior vs. schema validation.** Core does not execute or interpret `inputs` / `commands` at all — only `properties` drive `bind()`. Discovery still validates the schema of `inputs` / `commands` when present, however, so a malformed `attribute` or `async` field invalidates the declaration as a whole and — assuming `onUpdate` itself is valid (a non-function `onUpdate` MUST throw a `TypeError` synchronously before discovery at Level 2 / Core JS API compatibility, and SHOULD do so at Level 1O; see [SPEC.md § onUpdate validity](SPEC.md#onupdate-validity) for the level-by-level rule and why deferred detection silently accepts the bug for empty-properties targets) — `bind()` returns its no-op cleanup. The split is intentional: core stays narrow on *runtime behavior*, but the discovery contract is *well-formedness* — which has to be complete for downstream tools (codegen, remote proxy, devtools) to trust what they see.
