...
> Half of these rules have runnable starter vectors in [CONFORMANCE.md](CONFORMANCE.md). At minimum, an Extension 2 implementation should pass vectors **5, 6, 7, 9, 10, 15, 20, 21, 22, 23, 24, 25, 26, 27, 29, 30, and 34** (the `has`-trap pre-sync rule, the `update`-with-no-`value` undefined preservation, `JsonValue` validation, `setWithAck` ordering, the legacy-`setAck` rejection path, the `@wc-bindable/` reserved-prefix rule, the prototype-pollution reserved-names rule, duplicate-pending-`id` rejection, empty-string-`id` rejection, the malformed-`sync` ⇒ terminal rule, malformed-`return` / `throw` per-entry handling, `update.value` absence via key-presence, transport at-most-once delivery, **`dispose()` pending-drain in caller order with `WC_BINDABLE_DISPOSED`**, **cache validity across TerminalFailure / Reconnecting**, **locally-synthesized errors MUST carry `code`**, and **`set()` terminal vs transient throw behavior**), plus any core vectors that apply to the local bind surface it exposes — see CONFORMANCE.md's "Applies to" column for the per-vector scoping. The four newly-added entries (27, 29, 30, 34) are core safety vectors — `dispose` semantics, last-known-value cache across reconnect, the code-MUST contract that consumer recovery branches rely on, and the `set()` bifurcation that the entire fast-path-vs-acknowledged-path design rests on. An implementation that passes 5–26 but skips 27 / 29 / 30 / 34 is missing the failure / recovery surface the recent spec revisions formalized, so the minimum bar now includes them. Pass the applicable subset before claiming Extension 2 conformance; the file is necessary, not sufficient.
