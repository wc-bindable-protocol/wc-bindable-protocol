...
3. **`onClose` is at-most-once.** Reconnect is modeled as constructing a fresh transport instance and passing it to `reconnect()` on the proxy (an instance method on the object returned by `createRemoteCoreProxy()` — see § Naming below), not as re-firing `onClose` on the same instance.
