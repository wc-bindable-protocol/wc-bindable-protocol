...
| `WC_BINDABLE_PROTOCOL_ERROR` | Any other normatively-defined wire-protocol failure not covered above (malformed envelope shape, unexpected wire state, etc.) |
...
**Producer-side code-emission rule (origin-conditional MUST / MAY).** The `code` field is no longer uniformly OPTIONAL on the producer side — its required-ness depends on whether the error is **locally synthesized by the protocol** (the implementation already knows the failure class by construction) or **originated by application code on the producer** (the implementation cannot know the failure class without a guess):

| Error origin | Examples | `code` requirement on emission |
|---|---|---|
| **Locally synthesized protocol error** — the proxy / shell built the `Error` itself from a known protocol condition | `WC_BINDABLE_TIMEOUT` (AckOptions timer fired), `WC_BINDABLE_ABORTED` (AckOptions signal fired), `WC_BINDABLE_INVALID_JSON_VALUE` (consumer-side or producer-side `JsonValue` validation failed), `WC_BINDABLE_INVALID_RETURN_VALUE`, `WC_BINDABLE_UNDECLARED_INPUT` / `WC_BINDABLE_UNDECLARED_COMMAND`, `WC_BINDABLE_RESERVED_NAME`, `WC_BINDABLE_DUPLICATE_ID`, `WC_BINDABLE_PROTOCOL_ERROR` (malformed envelopes, unexpected wire state) | **MUST** carry `code`, drawn from the registry table above. Locally-synthesized errors are exactly the cases where the implementation has the failure class on hand at construction time, so omitting `code` here forces every consumer into `message`-string-pattern-matching — defeating the whole point of the registry. The "MUST" applies to both producer-side `throw` envelope emission AND consumer-side synthetic-error construction (e.g. on a malformed-`return` per § Consumer-side malformed message handling). |
| **Application error from the producer's command / setter** — `target[name] = value` or `target.command(...)` threw a non-protocol error | A user-defined validation error, a domain `RangeError`, a `fetch` failure inside a `fetch` command, anything thrown by application logic | `code` MAY be set to `WC_BINDABLE_REMOTE_THROW` (the safe default) or MAY be omitted; producers MAY also use an application-namespaced code (e.g. `MYAPP_VALIDATION_FAILED`) if they want a richer classification. **MUST NOT** repurpose a `WC_BINDABLE_*` code from the registry above for an application throw, because that would collide with the consumer's `code`-pattern-matching for protocol failures. The `name` / `message` (and optional `cause`) pair remains the canonical fallback for the no-code case. |

This split keeps the consumer-side recovery logic robust: a `try { await invoke(...) } catch (e) { switch (e.code) { case "WC_BINDABLE_TIMEOUT": ...; case "WC_BINDABLE_INVALID_JSON_VALUE": ... } }` block can rely on every `WC_BINDABLE_*` code being present whenever the failure class genuinely is one of those, without consumers having to pattern-match `e.message` substrings or fall back to `e.name` heuristics that legitimate runtime variation can break.

> **Backwards compatibility.** Older `@wc-bindable/remote` versions (pre-current spec) emitted `code` only on a subset of locally-synthesized errors. The strengthened MUST applies to **new producer implementations** and to the current reference implementation as it tracks toward full spec conformance; consumers MUST cope with `code` absence regardless, per the uniform consumer rule above. A producer that omits `code` on a locally-synthesized error is non-conformant under the current spec but does not break interop — the consumer falls back to `{ name, message }` matching. The rule exists so the fallback becomes the exception path, not the steady-state path.
...
- `N` absent from the new snapshot's `values` AND `undefinedProperties` AND `getterFailures` → the producer no longer surfaces this property at all. The cache entry for `N` MUST be removed; the `has`-trap MUST flip back to `false`. The proxy SHOULD NOT dispatch any synthetic event for the removal (no `undefined` ghost-event, no removal notification). Adapters that need to detect "the producer dropped property `N`" can compare against their own snapshot or read `proxy.constructor.wcBindable.properties` — the protocol does not surface property *removal* as a `bind()` event because there is no producer-side event to bridge from.
