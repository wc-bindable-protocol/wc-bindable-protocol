...
**Machine-readable `code` field.** The error envelope MAY carry an optional `code: string` whose value identifies the failure class in a form that survives `name` differences across runtimes (JS engines, ports to other languages) and message-text localization. `code` is OPTIONAL on both producer and consumer sides — producers MAY omit it; consumers MUST cope with its absence and treat the `{ name, message }` pair as the canonical fallback. When emitted, `code` SHOULD be drawn from the following normatively-registered set so cross-implementation pattern-matching works uniformly:
...
| `WC_BINDABLE_DUPLICATE_ID` | The producer received a `cmd` / id-bearing `set` whose `id` matches a pending entry — see the id-constraints bullet in § Message types — client → server |
...
- `cmd.id` is a client-allocated identifier (e.g. UUID v4). The producer MUST echo it back in the `return` / `throw` envelope. The following constraints are normative:
  ...
  - On the producer side, if an inbound `cmd` or id-bearing `set` carries an `id` that matches an entry the producer has not yet settled (return/throw not yet emitted), the producer **MUST** treat the later message as malformed and emit a `throw` envelope referencing that `id` with a clear error. The producer MUST NOT silently drop, MUST NOT apply the later message's effect, and MUST NOT cancel or re-settle the first pending entry. A duplicate `id` after the first one has been settled (i.e. `return`/`throw` already emitted for that `id`) is non-conformant consumer behavior but is handled symmetrically: the producer MUST `throw` against the duplicate (the consumer's `id → pending` table no longer carries the original entry, so it will treat the second `throw` as a normal late-resolution against a fresh pending — or as a late-envelope drop per § AckOptions if the consumer has already moved on). Producer-side id reuse detection is therefore part of the "malformed inbound" surface, not a separate state.
...
2. **Prototype-pollution-prone names.** The exact strings `"__proto__"`, `"constructor"`, and `"prototype"`. These are reserved at the same construction-time gate because the wire's value-bearing fields (`values`, `update.value`, `set.value`, `cmd.args[i]`, `return.value`, `throw.error.cause`) carry `{ [key: string]: JsonValue }` objects, and any implementation that materializes a `values` entry into a plain `{}` (e.g. via `Object.assign`, spread, or naive cache writes) will pollute the host object's prototype chain if a producer-declared property's name matches one of these. Rejecting them at construction time gives every conformant implementation the same defense without forcing every cache-write site to remember to use `Object.create(null)` or a `Map`. Implementations MAY *additionally* use null-prototype containers internally; the reservation is the simpler, single-checkpoint mitigation that interop tests can verify uniformly.

   The same names are reserved as **wire-payload keys** under § Design invariants invariant 3's existing object-shape rules (own enumerable string keys only; consumer-side handlers SHOULD use `Object.hasOwn` for key-presence checks, never `in`, as already done in § Consumer-side sync-response handling). A producer that emits a `values` / `update.value` object whose own enumerable key is `__proto__` / `constructor` / `prototype` is sending a payload whose declared origin is structurally impossible (those names cannot appear in any conformant declaration), so the consumer SHOULD treat the message as malformed per § Consumer-side malformed message handling and drop it.
