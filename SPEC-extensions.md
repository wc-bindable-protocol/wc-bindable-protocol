...
These are SHOULDs at the spec level — the protocol layer cannot make them MUSTs because the conformant transport surface is intentionally minimal — but they are operational MUSTs for any deployment crossing an untrusted boundary. Implementations of a `ServerTransport` or transport adapter that target untrusted use SHOULD document which of the above they enforce inline, which they expect the surrounding application to enforce, and which they cannot enforce from inside the transport boundary (per-command authorization, for example, typically lives in the Core's command implementation rather than the transport).

The above is **availability and confidentiality hardening, not protocol conformance.** A conformant `RemoteShellProxy` will work without any of these guardrails — it will also accept arbitrary `set` / `invoke` from any peer, with any `JsonValue` payload, at any rate. The conformance contract (per [Extension 2](#extension-2--wire-format-remote-proxying)) is what makes interoperability possible; this profile is what makes deploying it safely possible. The two layers compose; neither replaces the other.
