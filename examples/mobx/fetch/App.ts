...
export function mount(root: HTMLElement) {
  ...
  const url = observable.box("https://jsonplaceholder.typicode.com/posts/1");
  ...
  urlInput.addEventListener("input", () => {
    runInAction(() => url.set(urlInput.value));
  });
  ...
  autorun(() => {
    const loading = !!binder.state.loading;
  ...
  autorun(() => {
    statusStrong.textContent = String(binder.state.status || "—");
  });

  autorun(() => {
    const err = binder.state.error;
    if (err) {
