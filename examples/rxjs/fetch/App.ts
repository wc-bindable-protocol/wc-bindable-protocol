...
export function mount(root: HTMLElement) {
  ...
  const url = new BehaviorSubject("https://jsonplaceholder.typicode.com/posts/1");
  ...
  binder.subjects.loading.subscribe((loading) => {
    const isLoading = !!loading;
    loadingPill.style.cssText = `padding: 2px 8px; border-radius: 4px; background: ${isLoading ? "#fef3c7" : "#d1fae5"}; color: ${isLoading ? "#92400e" : "#065f46"}`;
    loadingStrong.textContent = String(isLoading);
    fetchBtn.disabled = isLoading;
  });

  binder.subjects.status.subscribe((status) => {
    statusStrong.textContent = String(status || "—");
  });

  binder.subjects.error.subscribe((err) => {
    if (err) {
  ...
  binder.subjects.value.subscribe((v) => {
