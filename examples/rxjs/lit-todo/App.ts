...
export function mount(root: HTMLElement) {
  ...
  listLabel.textContent = "RxJS-rendered item list (from bound state)";
  ...
  root.innerHTML = `
    <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 40px auto; padding: 0 20px">
      ...
      <h1>wc-bindable: RxJS — Lit Todo</h1>
      <div style="${card}">
  ...
  combineLatest([binder.subjects.items, binder.subjects.count]).subscribe(
    ([items, count]) => {
      valuesPre.textContent = JSON.stringify({ items, count }, null, 2);
    },
  );

  binder.subjects.items.subscribe((items) => {
    const list = items ?? [];
    listCard.style.display = list.length === 0 ? "none" : "";
