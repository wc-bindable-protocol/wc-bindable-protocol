...
export function mount(root: HTMLElement) {
  ...
  listLabel.textContent = "Signals-rendered item list (from bound state)";
  ...
  root.innerHTML = `
    <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 40px auto; padding: 0 20px">
      ...
      <h1>wc-bindable: Signals — Lit Todo</h1>
      <div style="${card}">
  ...
  effect(() => {
    const count = binder.signals.count.get();
    if (count != null) {
  ...
  effect(() => {
    valuesPre.textContent = JSON.stringify(
      {
        items: binder.signals.items.get(),
  ...
  effect(() => {
    const items = binder.signals.items.get() ?? [];
