...
describe("AiCore", () => {
  ...
  describe("provider", () => {
    ...
    it("不明なプロバイダ名でエラーをスローする", () => {
      const core = new AiCore();
      expect(() => { core.provider = "unknown"; }).toThrow('[@wc-bindable/ai] Unknown provider');
    });
  });
  ...
  describe("send (非ストリーミング)", () => {
    it("prompt未指定時にエラーをスローする", () => {
      const core = new AiCore();
      core.provider = "openai";
      expect(() => core.send("", { model: "gpt-4o" })).toThrow("[@wc-bindable/ai] prompt is required.");
    });

    it("provider未設定時にエラーをスローする", () => {
      const core = new AiCore();
      expect(() => core.send("Hello", { model: "gpt-4o" })).toThrow("[@wc-bindable/ai] provider is required.");
    });
    ...
    it("model未指定時にエラーをスローする", () => {
      ...
      expect(() => core.send("Hello", { model: "" })).toThrow(
        "[@wc-bindable/ai] model is required. See @wc-bindable/ai README §Supported Providers for each provider's model catalog (no default is shipped because model identifiers drift faster than library releases).",
      );
    });
