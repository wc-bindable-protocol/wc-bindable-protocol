...
export class Ai extends HTMLElement {
  ...
  private _initRemote(): void {
    ...
    if (!url) {
      throw new Error("[@wc-bindable/ai] remote.enableRemote is true but remoteCoreUrl is empty. Set remote.remoteCoreUrl or AI_REMOTE_CORE_URL environment variable.");
    }
    ...
    const onFail = () => {
      ...
      this._setErrorState(new Error(
        `[@wc-bindable/ai] WebSocket connection ${opened ? "lost" : "failed"}: ${url}`
      ));
  ...
  async send(): Promise<string | null> {
    ...
    if (!this._core) {
      throw this._errorState ?? new Error("[@wc-bindable/ai] Ai is not initialized yet. Attach the element to the DOM before calling send().");
    }
