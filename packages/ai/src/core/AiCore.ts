...
export class AiCore extends EventTarget {
  ...
  send(prompt: string | AiContentPart[], options: AiRequestOptions): Promise<string | null> {
    ...
    if (!options.model) raiseError("model is required. See @wc-bindable/ai README §Supported Providers for each provider's model catalog (no default is shipped because model identifiers drift faster than library releases).");
    ...
    try {
      while (true) {
        ...
        if (roundtrips > maxRoundtrips) {
          throw new Error(
            `[@wc-bindable/ai] maxToolRoundtrips (${maxRoundtrips}) exceeded; aborting tool-use loop.`,
          );
        }
