...
export function warnStreamParseFailure(
  ...
  console.warn("[@wc-bindable/ai] Failed to parse stream chunk.", {
    provider,
    event,
    data,
    error,
...
export function warnMalformedToolCall(
  ...
  console.warn("[@wc-bindable/ai] Dropped malformed tool_call accumulator (missing id or name).", {
    index,
    id: entry.id,
    name: entry.name,
...
export function warnApiKeyInRemoteMode(): void {
  ...
  console.warn(
    "[@wc-bindable/ai] `api-key` attribute is set on <ai-agent> in remote mode. " +
...
export function warnToolHandlerOverwrite(name: string): void {
  ...
  console.warn(
    `[@wc-bindable/ai] registerTool("${name}") replaced an existing handler with a different function reference. ` +
