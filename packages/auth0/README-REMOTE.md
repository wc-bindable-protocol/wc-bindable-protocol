...
In the HAWC taxonomy this is a thin browser-side gatekeeper that enables the **Case B** family: server-side Cores remain authoritative, while `<auth0-gate>` and `<auth0-session>` establish and expose the authenticated transport to the browser.
...
```bash
npm install @wc-bindable/auth0 @wc-bindable/remote @auth0/auth0-spa-js
```
...
```ts
import { registerCoreDeclaration } from "@wc-bindable/auth0";
...
```ts
import { registerCoreDeclaration, unregisterCoreDeclaration } from "@wc-bindable/auth0";
import { AppCore } from "./my-app-core.js";
