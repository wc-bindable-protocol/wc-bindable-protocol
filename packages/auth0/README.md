...
`@wc-bindable/auth0` is a headless authentication component for the wc-bindable ecosystem.
...
`@wc-bindable/auth0` follows the [HAWC](https://github.com/wc-bindable-protocol/wc-bindable-protocol/blob/main/packages/hawc/README.md) architecture:
...
`@wc-bindable/auth0` spans two HAWC shapes. In **local mode** it is the **Case A** shape: the Core itself is browser-anchored because the Auth0 SPA SDK and redirect callback flow require browser globals. In **remote mode** the element becomes a thin browser-side gatekeeper for authenticated access to server-side Cores, typically paired with `<auth0-session>`.
...
```bash
npm install @wc-bindable/auth0 @auth0/auth0-spa-js
```
