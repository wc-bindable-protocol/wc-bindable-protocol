...
describe("AuthCore", () => {
  ...
  describe("initialize", () => {
    it("domain未指定時にエラーをスローする", () => {
      ...
      expect(() => core.initialize({ domain: "", clientId: "id" })).toThrow(
        "[@wc-bindable/auth0] domain is required."
      );
    });
    ...
    it("clientId未指定時にエラーをスローする", () => {
      ...
      expect(() => core.initialize({ domain: "test.auth0.com", clientId: "" })).toThrow(
        "[@wc-bindable/auth0] clientId is required."
      );
    });
  ...
  describe("login", () => {
    it("クライアント未初期化時にエラーをスローする", async () => {
      const core = new AuthCore();
      await expect(core.login()).rejects.toThrow("[@wc-bindable/auth0] Auth0 client is not initialized.");
    });
  ...
  describe("loginWithPopup", () => {
    it("クライアント未初期化時にエラーをスローする", async () => {
      const core = new AuthCore();
      await expect(core.loginWithPopup()).rejects.toThrow("[@wc-bindable/auth0] Auth0 client is not initialized.");
    });
  ...
  describe("logout", () => {
    it("クライアント未初期化時にエラーをスローする", async () => {
      const core = new AuthCore();
      await expect(core.logout()).rejects.toThrow("[@wc-bindable/auth0] Auth0 client is not initialized.");
    });
  ...
  describe("getToken", () => {
    it("クライアント未初期化時にエラーをスローする", async () => {
      const core = new AuthCore();
      await expect(core.getToken()).rejects.toThrow("[@wc-bindable/auth0] Auth0 client is not initialized.");
    });
