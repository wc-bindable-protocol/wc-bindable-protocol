 ...
 * `Error` so subscribers of `hawc-auth0:error` observe a uniform
...
export class AuthCore extends EventTarget {
   ...
   *     `<hawc-auth0>`'s connectedCallback guard
  ...
  initialize(options: Auth0ClientOptions): Promise<void> {
    // Phrased in terms of the `options` field rather than the `<hawc-auth0>`
   ...
   * `<hawc-auth0>.login()` awaits `connectedCallbackPromise` before
