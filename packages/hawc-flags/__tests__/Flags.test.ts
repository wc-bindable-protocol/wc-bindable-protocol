...
// <hawc-flags>. Holds a proxy (EventTarget with getters + invoke()),
// a `ready` boolean, and fires `hawc-auth0-session:ready-changed`.
...
function makeFlagsEl(target = "sess"): Flags {
  const el = document.createElement("hawc-flags") as Flags;
  el.target = target;
...
describe("<hawc-flags>", () => {
  ...
  it("registered as a custom element", () => {
    expect(customElements.get("hawc-flags")).toBeDefined();
  });
  ...
  it("recovers when the target session is inserted AFTER <hawc-flags>", async () => {
    // Regression: framework / SSR-hydrate / async-mount ordering
    // sometimes lands <hawc-flags> in the DOM before its target
  ...
  it("the rescue observer disconnects on disconnectedCallback", async () => {
    ...
    let hits = 0;
    el.addEventListener("hawc-flags:flags-changed", () => hits++);
  ...
  it("recovers when the target element exists but its proxy/ready are ASSIGNED in place later", async () => {
    // Regression: `<hawc-flags>`'s structural contract is "any
    ...
    // SDK wiring) was leaving <hawc-flags> permanently stuck.
    vi.useFakeTimers();
    try {
      // Plain container present BEFORE <hawc-flags>, but lacking
  ...
  it("binds to proxy once the target becomes ready", async () => {
    ...
    await flush();
    const seen: FlagMap[] = [];
    el.addEventListener("hawc-flags:flags-changed", (e) => {
  ...
  it("re-dispatches identified / loading / error changes", async () => {
    ...
    for (const type of ["hawc-flags:identified-changed", "hawc-flags:loading-changed", "hawc-flags:error"]) {
    ...
    expect(events.some((e) => e.type === "hawc-flags:identified-changed" && e.detail === true)).toBe(true);
    expect(events.some((e) => e.type === "hawc-flags:loading-changed" && e.detail === true)).toBe(true);
  });
  ...
  it("clearing error back to null is idempotent", async () => {
    ...
    await flush();
    let nullCount = 0;
    el.addEventListener("hawc-flags:error", (e) => {
  ...
  it("identified-changed is deduped when the same value is re-emitted", async () => {
    ...
    await flush();
    let hits = 0;
    el.addEventListener("hawc-flags:identified-changed", () => hits++);
    sess.proxy!._publish("identified", true);
    sess.proxy!._publish("identified", true);
    expect(hits).toBe(1);
  });
  ...
  it("loading-changed is deduped", async () => {
    ...
    await flush();
    let hits = 0;
    el.addEventListener("hawc-flags:loading-changed", () => hits++);
    sess.proxy!._publish("loading", true);
    sess.proxy!._publish("loading", true);
    expect(hits).toBe(1);
  });
  ...
  it("ignores ready-changed events with non-boolean details", async () => {
    ...
    sess.dispatchEvent(new CustomEvent("hawc-auth0-session:ready-changed", {
      detail: "weird",
      bubbles: true,
    }));
    expect(el.flags).toEqual({});
