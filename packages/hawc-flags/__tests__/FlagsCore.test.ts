...
function eventRecorder(core: FlagsCore) {
  ...
  const types = [
    "hawc-flags:flags-changed",
    "hawc-flags:identified-changed",
    "hawc-flags:loading-changed",
    "hawc-flags:error",
  ];
...
describe("FlagsCore", () => {
  describe("construction", () => {
    ...
    it("dispatches events on the injected target", () => {
      ...
      const events: string[] = [];
      target.addEventListener("hawc-flags:flags-changed", () => events.push("flags"));
  ...
  describe("identify", () => {
    ...
    it("publishes the initial flag snapshot and flips identified", async () => {
      ...
      expect(types).toContain("hawc-flags:loading-changed");
      expect(types).toContain("hawc-flags:flags-changed");
      expect(types).toContain("hawc-flags:identified-changed");
    });
    ...
    it("provider push updates re-publish flags", async () => {
      ...
      core.addEventListener("hawc-flags:flags-changed", (e) => {
    ...
    it("publishes an error when the provider rejects identify", async () => {
      ...
      expect(core.identified).toBe(false);
      expect(core.loading).toBe(false);
      const errEvents = events.filter((e) => e.type === "hawc-flags:error" && e.detail);
    ...
    it("a failed FIRST identify does not emit a spurious flags-changed event", async () => {
      ...
      core.addEventListener("hawc-flags:flags-changed", (e) => events.push((e as CustomEvent).detail));
  ...
  describe("event dedupe", () => {
    it("identified-changed does not fire twice for the same value", async () => {
      ...
      const core = new FlagsCore({ provider: p });
      const events: unknown[] = [];
      core.addEventListener("hawc-flags:identified-changed", (e) => {
    ...
    it("loading-changed toggles cleanly across identify cycles", async () => {
      ...
      core.addEventListener("hawc-flags:loading-changed", (e) => {
    ...
    it("error event does not re-fire when the null state is already null", async () => {
      ...
      core.addEventListener("hawc-flags:error", (e) => {
