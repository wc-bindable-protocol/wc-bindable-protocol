...
/**
 * Element structurally matching `<hawc-auth0-session>`'s public surface.
 * Any element exposing `.proxy`, `.ready`, and the
 * `hawc-auth0-session:ready-changed` event works as a target — we do
 * not take a hard dependency on `@wc-bindable/hawc-auth0` so tests
...
/**
 * `<hawc-flags>` — declarative feature-flag observation shell.
 *
 * Subscribes to the flag-shaped bindable surface of a session element
 * (typically `<hawc-auth0-session>`) and re-dispatches those property
 ...
 *
 *   <hawc-auth0 id="auth" ... />
 *   <hawc-auth0-session target="auth" core="flags-core" id="auth-session" />
 *   <hawc-flags
 *     target="auth-session"
...
export class Flags extends HTMLElement {
  static wcBindable: IWcBindable = {
    ...
    version: 1,
    properties: [
      { name: "flags",      event: "hawc-flags:flags-changed" },
      { name: "identified", event: "hawc-flags:identified-changed" },
      { name: "loading",    event: "hawc-flags:loading-changed" },
      { name: "error",      event: "hawc-flags:error" },
    ],
    commands: [
  ...
  async identify(userId: string, attrs?: Record<string, unknown>): Promise<void> {
    ...
    if (!proxy) {
      throw new Error(
        "[@wc-bindable/hawc-flags] <hawc-flags>: identify() called before a session proxy is attached. Wait for the target session's ready=true.",
      );
    }
  ...
  async reload(): Promise<void> {
    ...
    if (!proxy) {
      throw new Error(
        "[@wc-bindable/hawc-flags] <hawc-flags>: reload() called before a session proxy is attached. Wait for the target session's ready=true.",
      );
    }
  ...
  private _attach(): void {
    ...
    if (!session) {
      this._setError(new Error(
        `[@wc-bindable/hawc-flags] <hawc-flags>: target "${this.target}" did not resolve to a session element exposing \`.proxy\`.`,
      ));
    ...
    session.addEventListener("hawc-auth0-session:ready-changed", listener);
  ...
  private _detach(): void {
    ...
    if (this._sessionEl && this._readyListener) {
      this._sessionEl.removeEventListener("hawc-auth0-session:ready-changed", this._readyListener);
    }
   ...
   * one. Without this rescue, a framework that mounts `<hawc-flags>`
  ...
  private _setIdentified(value: boolean): void {
    ...
    this.dispatchEvent(new CustomEvent("hawc-flags:identified-changed", {
      detail: value,
      bubbles: true,
    }));
  }
  ...
  private _setLoading(value: boolean): void {
    ...
    this._loading = value;
    this.dispatchEvent(new CustomEvent("hawc-flags:loading-changed", {
      detail: value,
      bubbles: true,
    }));
  }
