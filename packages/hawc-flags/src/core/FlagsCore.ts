...
export class FlagsCore extends EventTarget {
  ...
  /**
   * Propagate a refreshed `UserContext` (typically fired by hawc-auth0's
  ...
  private _publishFlags(next: FlagMap): void {
    ...
    this._target.dispatchEvent(new CustomEvent("hawc-flags:flags-changed", {
      detail: snapshot,
      bubbles: true,
    }));
  }
  ...
  private _setIdentified(value: boolean): void {
    ...
    this._target.dispatchEvent(new CustomEvent("hawc-flags:identified-changed", {
      detail: value,
      bubbles: true,
    }));
  }
  ...
  private _setLoading(value: boolean): void {
    ...
    this._loading = value;
    this._target.dispatchEvent(new CustomEvent("hawc-flags:loading-changed", {
      detail: value,
      bubbles: true,
    }));
  }
...
/**
 * Flatten a hawc-auth0 `UserContext` into a {@link FlagIdentity} using
