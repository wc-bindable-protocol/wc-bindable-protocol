...
describe("S3Core", () => {
  ...
  it("abort() after a successful upload clears completed=true", async () => {
    ...
    // firing `hawc-s3:completed-changed`.
    await core.requestUpload("k", 1);
    await core.complete("k", "etag");
    expect(core.completed).toBe(true);

    const events: boolean[] = [];
    core.addEventListener("hawc-s3:completed-changed", (e) => {
  ...
  it("abort() after a successful upload clears url/etag/metadata to match completed=false", async () => {
    ...
    core.addEventListener("hawc-s3:url-changed", (e) => urlEvents.push((e as CustomEvent).detail));
    core.addEventListener("hawc-s3:etag-changed", (e) => etagEvents.push((e as CustomEvent).detail));
    core.addEventListener("hawc-s3:metadata-changed", (e) => metadataEvents.push((e as CustomEvent).detail));
  ...
  describe("gen mismatch on complete keeps state consistent for the next upload", () => {
    ...
    // run would lie to any binding subscribed to `hawc-s3:completed-changed`
  ...
  describe("generation race protection", () => {
    ...
    it("deleteObject on an active upload key dispatches hawc-s3:delete-during-upload-warning", async () => {
      core.bucket = "b";
      await core.requestUpload("live-key", 5);
      expect(core.uploading).toBe(true);

      const warnings: CustomEvent[] = [];
      core.addEventListener("hawc-s3:delete-during-upload-warning", (e) => {
    ...
    it("abort() surfaces abortMultipart failures via hawc-s3:abort-cleanup-warning", async () => {
      ...
      const warnings: CustomEvent[] = [];
      core.addEventListener("hawc-s3:abort-cleanup-warning", (e) => {
        warnings.push(e as CustomEvent);
      });
