...
export function validateRemoteCoreUrl(url: string): URL {
  ...
  try {
    ...
    throw new Error(`[@wc-bindable/hawc-s3] remote.remoteCoreUrl is not a valid URL: ${url}`);
  }
  if (parsed.protocol !== "ws:" && parsed.protocol !== "wss:") {
    throw new Error(`[@wc-bindable/hawc-s3] remote.remoteCoreUrl must use ws:// or wss:// scheme, got ${parsed.protocol} (${url}).`);
  }
  return parsed;
}
