...
export class SkewError extends Error {
  ...
  constructor(skewMs: number, allowedMs: number) {
    super(`[@wc-bindable/hawc-s3] clock skew ${skewMs}ms exceeds allowable ${allowedMs}ms (signer clock vs reference).`);
    this.name = "SkewError";
    this.skewMs = skewMs;
...
export async function presignS3Url(
  ...
  if (!Number.isFinite(rawExpires)) {
    throw new Error(`[@wc-bindable/hawc-s3] expiresInSeconds must be a finite number, got ${rawExpires}.`);
  }
  if (rawExpires > 604800) {
    throw new Error(`[@wc-bindable/hawc-s3] expiresInSeconds exceeds AWS limit of 604800 (7 days), got ${rawExpires}.`);
  }
  ...
  if (params.extraQuery) {
    for (const k of Object.keys(params.extraQuery)) {
      if (RESERVED_SIGV4_QUERY_KEYS.has(k.toLowerCase())) {
        throw new Error(`[@wc-bindable/hawc-s3] extraQuery must not include reserved SigV4 parameter "${k}".`);
      }
    }
  }
