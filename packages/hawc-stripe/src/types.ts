 ...
 * as `unknown` upstream and narrowed by the app — hawc-stripe does not read
 ...
 * apps downcast in their registered handler. Kept minimal so hawc-stripe does
 ...
 * a `hawc-stripe:webhook-warning` event on the Core's target and the chain
 * continues. Mirrors hawc-s3's registerPostProcess fatal semantics.
 */
 ...
 * imperatively fire `submit()` via a reactive boolean (mirrors hawc-s3).
 */
