...
describe("WebAuthnCore", () => {
  ...
  describe("createRegistrationChallenge", () => {
    ...
    it("transitions status to challenging and sets user", async () => {
      const events: any[] = [];
      core.addEventListener("hawc-webauthn:status-changed", (e: any) => events.push(["status", e.detail]));
      core.addEventListener("hawc-webauthn:user-changed", (e: any) => events.push(["user", e.detail]));
    ...
    it("does not redispatch status when it is already challenging", async () => {
      ...
      core.addEventListener("hawc-webauthn:status-changed", (e: any) => statuses.push(e.detail));
      await core.createRegistrationChallenge("s1", { id: "u", name: "u@x", displayName: "U" });
  ...
  describe("target injection", () => {
    it("dispatches events on the external target when provided", async () => {
      ...
      const events: string[] = [];
      target.addEventListener("hawc-webauthn:status-changed", (e: any) => events.push(e.detail));
