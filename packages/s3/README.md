...
`@wc-bindable/s3` is a headless **S3 (and S3-compatible) blob store** component built on wc-bindable-protocol.
...
`@wc-bindable/s3` follows the [HAWC](https://github.com/wc-bindable-protocol/wc-bindable-protocol/blob/main/packages/hawc/README.md) architecture:
...
`@wc-bindable/s3` moves all of that behind two custom elements and a server-side `S3Core`,
...
<script type="module" src="/node_modules/@wc-bindable/s3/src/auto/remoteEnv.min.js"></script>
...
`<s3-uploader>` is HAWC + `wc-bindable-protocol`, so it works with any framework
...
import type { WcsS3Values } from "@wc-bindable/s3";
...
design. The intended path is a separate `@wc-bindable/s3-resumable`
...
|---|---|---|
| `@wc-bindable/s3` | browser | `bootstrapS3`, `WcsS3`, `WcsS3Callback`, `S3Core`, `AwsS3Provider`, retry helpers, types |
| `@wc-bindable/s3/server` | Node | `S3Core`, `AwsS3Provider`, `presignS3Url`, retry helpers, types — **no `HTMLElement`-based code** |
| `@wc-bindable/s3/auto` | browser (side-effect) | calls `bootstrapS3()` so `<s3-uploader>` and `<s3-callback>` are registered on import |
| `@wc-bindable/s3/auto/remoteEnv` | browser (side-effect) | calls `bootstrapS3({ remote: { enableRemote: true, remoteSettingType: "env" } })` — reads the WS URL from `globalThis.S3_REMOTE_CORE_URL` or `process.env.S3_REMOTE_CORE_URL` |
