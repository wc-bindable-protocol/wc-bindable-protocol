...
// Register the host element under TWO tag names: the default s3-uploader (so other
...
beforeAll(() => {
  ...
  if (!customElements.get("s3-callback")) customElements.define("s3-callback", S3Callback);
...
describe("S3Callback host lookup respects config.tagNames", () => {
  it("default tag name: callback finds the ancestor <s3-uploader>", async () => {
    setConfig({ tagNames: { s3: "s3-uploader", s3Callback: "s3-callback" } });
    const host = document.createElement("s3-uploader");
    host.setAttribute("bucket", "b");
    const cb = document.createElement("s3-callback");
  ...
  it("custom tag name: callback walks up to a renamed host element", async () => {
    setConfig({ tagNames: { s3: CUSTOM_HOST, s3Callback: "s3-callback" } });
  ...
  it("custom tag name: callback does NOT spuriously match the default tag", async () => {
    // After setConfig switches to a custom host tag, a leftover <s3-uploader>
    // ancestor must not be picked up — the contract is "configured tag only."
    setConfig({ tagNames: { s3: CUSTOM_HOST, s3Callback: "s3-callback" } });
    const wrongHost = document.createElement("s3-uploader");
    const cb = document.createElement("s3-callback");
    cb.setAttribute("on", "completed");
    cb.innerHTML = `<script type="module">export default () => {}</script>`;
  ...
  it("`for` selector still works regardless of tagNames config", async () => {
    setConfig({ tagNames: { s3: CUSTOM_HOST, s3Callback: "s3-callback" } });
    const host = document.createElement("s3-uploader"); // intentionally NOT the configured tag
...
describe("S3Callback async-attach race", () => {
  ...
  it("connect-then-immediately-disconnect does NOT register a listener on the host", async () => {
    ...
    const callbackEventCalls = addSpy.mock.calls.filter(
      ([name]) => typeof name === "string" && name.startsWith("s3-uploader:"),
    );
  ...
  it("attaches normally when the element stays connected", async () => {
    ...
    const callbackEventCalls = addSpy.mock.calls.filter(
      ([name]) => typeof name === "string" && name === "s3-uploader:completed-changed",
    );
