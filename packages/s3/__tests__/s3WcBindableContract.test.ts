...
describe("S3 wcBindable contract", () => {
  ...
  it("every declared command exists as a callable method on instances", () => {
    ...
    const el = document.createElement("s3-uploader") as unknown as Record<string, unknown>;
    for (const cmd of S3.wcBindable.commands ?? []) {
      expect(
        typeof el[cmd.name],
        `<s3-uploader> advertises command "${cmd.name}" but no method of that name is callable on the element`,
      ).toBe("function");
    }
  });
