...
describe("resolveSafeStaticPath", () => {
  describe("legitimate paths", () => {
    it("resolves a normal file path under the root", () => {
      const r = resolveSafeStaticPath(ROOT, "/s3-uploader/dist/index.js");
      expect(r).toBe(path.join(ROOT, "s3-uploader/dist/index.js"));
    });
    ...
    it("decodes percent-encoded path segments before resolving", () => {
      const r = resolveSafeStaticPath(ROOT, "/s3-uploader%2Fdist/index.js");
      expect(r).toBe(path.join(ROOT, "s3-uploader/dist/index.js"));
    });

    it("ignores trailing query and fragment", () => {
      const r = resolveSafeStaticPath(ROOT, "/s3-uploader/dist/index.js?v=1#frag");
      expect(r).toBe(path.join(ROOT, "s3-uploader/dist/index.js"));
    });
  });
  ...
  describe("traversal rejected", () => {
    ...
    it("rejects partial traversal that lands back inside (defense in depth)", () => {
      // /s3-uploader/../../package.json normalizes to /package.json (one above
      ...
      expect(resolveSafeStaticPath(ROOT, "/s3-uploader/../../package.json")).toBeNull();
    });
    ...
    it("rejects null-byte injection", () => {
      ...
      expect(resolveSafeStaticPath(ROOT, "/s3-uploader/dist/index.js\0../package.json")).toBeNull();
    });
  ...
  describe("regression: naive path.join() WOULD have leaked these", () => {
    ...
    it("/packages/s3/../../package.json equivalent", () => {
      expect(resolveSafeStaticPath(ROOT, "/s3-uploader/../../package.json")).toBeNull();
    });
  });
});
...
describe("isAllowedPackageAsset", () => {
  ...
  it("allows compiled output under <pkg>/dist/**", () => {
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/dist/index.js"))).toBe(true);
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/dist/providers/AwsS3Provider.js"))).toBe(true);
    expect(isAllowedPackageAsset(PACKAGES, at("core/dist/index.js"))).toBe(true);
  });

  it("rejects TypeScript source under <pkg>/src/**", () => {
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/src/components/S3.ts"))).toBe(false);
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/src/signing/sigv4.ts"))).toBe(false);
  });

  it("rejects package metadata (package.json, tsconfig.json)", () => {
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/package.json"))).toBe(false);
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/tsconfig.json"))).toBe(false);
  });

  it("rejects test directories", () => {
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/__tests__/sigv4.test.ts"))).toBe(false);
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/tests/integration/server.ts"))).toBe(false);
  });

  it("rejects a file that sits directly under a package root", () => {
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/README.md"))).toBe(false);
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/LICENSE"))).toBe(false);
  });

  it("rejects nested node_modules", () => {
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader/node_modules/some-lib/lib.js"))).toBe(false);
  });

  it("rejects the packages root itself and a bare package dir", () => {
    expect(isAllowedPackageAsset(PACKAGES, PACKAGES)).toBe(false);
    expect(isAllowedPackageAsset(PACKAGES, at("s3-uploader"))).toBe(false);
  });
  ...
  it("rejects paths that are not under packagesRoot at all", () => {
    ...
    expect(isAllowedPackageAsset(PACKAGES, path.resolve("/srv/other/s3-uploader/dist/index.js"))).toBe(false);
  });
});
