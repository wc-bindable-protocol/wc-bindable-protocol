...
export class S3 extends HTMLElement {
  ...
  private _initRemote(): void {
    ...
    const onFail = () => {
      ...
      this._setErrorState(new Error(
        `[@wc-bindable/s3] WebSocket connection ${opened ? "lost" : "failed"}: ${url}`
      ));
  ...
  _connectRemote(transport: ClientTransport): void {
    ...
    if (this._core) {
      throw new Error("[@wc-bindable/s3] _connectRemote() called while a local Core is already attached. Call disconnectedCallback / detach first.");
    }
  ...
  attachLocalCore(core: S3Core): void {
    ...
    if (this._proxy) {
      throw new Error("[@wc-bindable/s3] attachLocalCore() called while a remote proxy is already attached. Call disconnectedCallback / detach first.");
    }
    this._core = core;
  ...
  private async _dispatchAsync<T>(
    ...
    if (!this._core) throw new Error("[@wc-bindable/s3] no core attached.");
  ...
  async upload(): Promise<string | null> {
    ...
    if (!file) {
      const err = new Error("[@wc-bindable/s3] file is required. Set the .file property before triggering upload.");
  ...
  private async _doMultipart(key: string, file: Blob, contentType: string | undefined): Promise<string | null> {
    ...
    if (this._aborted || firstError) {
      ...
      const err = firstError ?? new Error("[@wc-bindable/s3] upload aborted.");
