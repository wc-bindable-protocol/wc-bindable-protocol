...
export class S3Core extends EventTarget {
  ...
  async requestMultipartUpload(
    ...
    if (partCount > S3_MAX_PARTS) {
      ...
      const e = new Error(`[@wc-bindable/s3] computed part count ${partCount} exceeds S3 max ${S3_MAX_PARTS}.`);
      this._setError(e);
      this._setLoading(false);
      throw e;
    }
    ...
    if (gen !== this._generation) {
      ...
      throw new Error("[@wc-bindable/s3] requestMultipartUpload superseded.");
    }
