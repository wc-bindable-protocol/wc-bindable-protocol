...
if (!customElements.get("passkey-auth-wire")) {
  customElements.define("passkey-auth-wire", class extends WebAuthn {});
}
...
describe("wire boundary: Core ↔ handlers ↔ Shell", () => {
  ...
  it("survives a registration when user-id contains non-base64url characters", async () => {
    ...
    const el = document.createElement("passkey-auth-wire") as WebAuthn;
